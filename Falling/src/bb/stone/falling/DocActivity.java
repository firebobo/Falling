...
public class DocActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		bt.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				text.setText(manager.size() + ","+arrayData.size());
			}
		});
	...
	BroadcastReceiver sensorReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			SensorData data = new SensorData(ts, xA, yA, zA, xD, yD, zD);
			if (arrayData.size() > 0) {
					...
					public void run() {
						...
						try {
							for (int i = 0; i < temp.size(); i++) {
								...
								copyBigDataToSD(dirPath, temp.get(i).toString()+"\n");
	...
	public void saveAppend(String filename, String content) throws Exception {
		...
		FileOutputStream outStream = openFileOutput(fullFilename,
				Context.MODE_APPEND);
	...
	public void unLock() {
		...
		setLockPatternEnabled(
				android.provider.Settings.Secure.LOCK_PATTERN_ENABLED, false,
				mContentResolver);
	}

	private void setLockPatternEnabled(String systemSettingKey,
			boolean enabled, ContentResolver mContentResolver) {
		// 不建议使用
	...
	private void copyBigDataToSD(String strOutFileName, String data)
			throws IOException {

		ByteArrayInputStream myInput;

		FileOutputStream myOutput = new FileOutputStream(strOutFileName, true);
